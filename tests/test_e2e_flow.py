...
def test_full_flow() -> None:
    ...
    with sync_playwright() as p:
        ...
        initial_row_count = bucket_table.locator("tbody tr").count()
        ...
        # ----------------------------
        add_candidates_button = page.get_by_role(
            "button",
            name=re.compile(r"add candidates", re.I),
        )

        expect(add_candidates_button).to_be_visible(timeout=15000)
        add_candidates_button.click()

        expect(
            page.get_by_role("heading", name=re.compile(r"add candidates", re.I))
